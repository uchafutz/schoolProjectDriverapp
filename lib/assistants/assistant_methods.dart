...
class AssistantMethods {
  ...
  static double calculateFareAmountFromOriginToDestination(
    ...
    double timeTraveledFareAmountPerMinute =
        (directionDetailsInfo.duration_value! / 60) * 0.54;
    double distanceTraveledFareAmountPerKilometer =
        (directionDetailsInfo.duration_value! / 1000) * 0.54;

    double localFareAmountTotal = timeTraveledFareAmountPerMinute +
        distanceTraveledFareAmountPerKilometer;

    // 1 usd= 2328.00
    double totalFareAmount = localFareAmountTotal * 2328.00;

    if (driverVehicleType == "toyo") {
      double resultFareAmount = (totalFareAmount.truncate()) / 2.0;
      return resultFareAmount;
    } else if (driverVehicleType == "canter") {
      return totalFareAmount.truncate().toDouble();
    } else if (driverVehicleType == "fuso") {
      double resultFareAmount = (totalFareAmount.truncate()) * 2.0;
      return resultFareAmount;
    } else {
      return totalFareAmount.truncate().toDouble();
